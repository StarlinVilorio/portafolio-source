...
export function VisualEffects() {
    ...
    useEffect(() => {
        ...
        const scroll = () => {
            ...
            scrollFrame = requestAnimationFrame(() => {
                ...
                if (progress.current)
                    progress.current.style.transform = `scaleX(${max > 0 ? Math.min(1, Math.max(0, window.scrollY / max)) : 0})`;
                scrollFrame = 0;
            });
        };
        const reset = () => {
    ...
    <button type="button" className="motion-control" onClick={() => setPaused(!paused)} disabled={reduced} aria-pressed={!disabled} aria-label={t(reduced ? 'Movimiento reducido según tu sistema' : paused ? 'Activar efectos visuales' : 'Pausar efectos visuales')}>
      {disabled ? <Play size={14}/> : <Pause size={14}/>}
      <span>{t(reduced ? 'Movimiento reducido' : paused ? 'Activar efectos' : 'Pausar efectos')}</span>
    </button>
  </>;
}
